...
class ArchitectureDetector(private val projectRoot: String) {
    ...
    fun detect(): ArchitectureResult {
        ...
        log.info("[ARCH] Detected: {} (confidence: {:.2f}, indicators: {})", 
        ...
        moduleRoot.walk()
            ...
            .forEach { file ->
                when (file.extension) {
                    ...
                    "ts", "tsx" -> languageCounts[Language.TYPESCRIPT] = languageCounts.getOrDefault(Language.TYPESCRIPT, 0) + 1
                    "js", "jsx" -> languageCounts[Language.JAVASCRIPT] = languageCounts.getOrDefault(Language.JAVASCRIPT, 0) + 1
    ...
    private fun detectPlatformsInModule(moduleRoot: File, frameworks: List<Framework>): List<Platform> {
        ...
        // Backend detection
        if (frameworks.any { it in listOf(Framework.SPRING_BOOT, Framework.KTOR, Framework.MICRONAUT, Framework.QUARKUS) }) {
    ...
    private fun detectArchitecturePatterns(modules: List<Module>, indicators: MutableList<String>): List<ArchitecturePattern> {
    ...
    private fun buildEntryPointPatterns(
        ...
        frameworks.forEach { framework ->
            when (framework) {
                Framework.SPRING_BOOT -> entryPatterns.addAll(listOf(
                    "@RestController", "@Controller", "@Service", "@Repository", "@Component"
                ))
                Framework.KTOR -> entryPatterns.addAll(listOf(
                    "routing\\s*\\{", "fun\\s+Application\\.\\w+\\(", "fun\\s+Route\\.\\w+\\("
                ))
                Framework.REACT -> entryPatterns.addAll(listOf(
                    "export\\s+(default\\s+)?function", "export\\s+const\\s+\\w+\\s*="
                ))
                Framework.CLIKT, Framework.PICOCLI -> entryPatterns.addAll(listOf(
                    "class\\s+\\w+Command", "fun\\s+main\\s*\\("
                ))
                Framework.AGENT_FRAMEWORK -> entryPatterns.addAll(listOf(
        ...
        platforms.forEach { platform ->
            when (platform) {
                Platform.BACKEND -> entryPatterns.addAll(listOf(
                    "class\\s+\\w+Controller", "class\\s+\\w+Service", "interface\\s+\\w+Repository"
                ))
                Platform.CLI -> entryPatterns.addAll(listOf(
                    "fun\\s+main\\s*\\(", "class\\s+\\w+Command"
                ))
                Platform.LIBRARY -> entryPatterns.addAll(listOf(
        ...
        patterns.forEach { pattern ->
            when (pattern) {
                ArchitecturePattern.CLEAN -> entryPatterns.addAll(listOf(
                    "interface\\s+\\w+UseCase", "class\\s+\\w+UseCase"
                ))
                ArchitecturePattern.HEXAGONAL -> entryPatterns.addAll(listOf(
                    "interface\\s+\\w+Port", "class\\s+\\w+Adapter"
                ))
                ArchitecturePattern.EVENT_DRIVEN -> entryPatterns.addAll(listOf(
