...
class ContextIntegrationTest {
    ...
    fun `should return error for enhanced context without cache`() = withTempDir { tempDir ->
        ...
        val result = I2VisionCli().testing(
            "context", "enhanced",
        ...
        assertTrue(
            result.output.contains("No discovery cache found") || result.output.contains("Error"),
    ...
    fun `should return basic context without discovery cache`() = withTempDir { tempDir ->
        ...
        assertTrue(
            result.output.contains("Symbols") || result.output.contains("symbol"),
            "Should show symbols"
        )
        assertTrue(
            result.output.contains("[BASIC]") || result.output.contains("Basic context"),
    ...
    fun `should show cache stats`() = withTempDir { tempDir ->
        ...
        assertTrue(result.statusCode == 0, "Should return zero exit code")
        assertTrue(
            result.output.contains("Cache") || result.output.contains("entries"),
    ...
    fun `should handle cache clean command`() = withTempDir { tempDir ->
        ...
        assertTrue(result.statusCode == 0, "Should return zero exit code")
        assertTrue(
            result.output.contains("cleaned") || result.output.contains("success"),
    ...
    fun `should require pattern for cache invalidate`() = withTempDir { tempDir ->
        ...
        assertTrue(result.statusCode != 0, "Should return non-zero exit code")
        assertTrue(
            result.output.contains("pattern") || result.output.contains("required"),
