...
class FlowDiscovery(
    ...
    fun discoverFlows(
        ...
        val callGraph = try {
            ...
            log.error(
                "[FLOW_DISCOVERY] Failed to build call graph for cluster '{}': {}",
                clusterId ?: "unknown",
                e.message
            )
        ...
        if (callGraph.isEmpty()) {
            log.warn(
                "[FLOW_DISCOVERY] No call graph nodes found for cluster '{}', skipping flow discovery",
                clusterId ?: "unknown"
            )
            return emptyList()
        }
        ...
        if (entryPoints.isEmpty()) {
            log.warn(
                "[FLOW_DISCOVERY] No entry points found for cluster '{}', skipping flow discovery",
                clusterId ?: "unknown"
            )
            return emptyList()
        }
    ...
    private fun buildCallGraph(
        symbols: List<SymbolInfo>,
        fileContentCache: MutableMap<File, String>
     ...
     */
    private fun findEntryPoints(
        symbols: List<SymbolInfo>,
        fileContentCache: MutableMap<File, String> = mutableMapOf()
    ): List<SymbolInfo> {
