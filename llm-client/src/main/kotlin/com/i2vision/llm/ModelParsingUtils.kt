...
object ModelParsingUtils {
     ...
     */
    fun parseOllamaGenerateTokenCounts(chunk: JsonObject?, fallbackUsed: Int, fallbackGenerated: Int): Pair<Int, Int> {
        val used = chunk?.get("prompt_eval_count")?.jsonPrimitive?.content?.toIntOrNull()
        val gen = chunk?.get("eval_count")?.jsonPrimitive?.content?.toIntOrNull()
        return Pair(used ?: fallbackUsed, gen ?: fallbackGenerated)
    }
